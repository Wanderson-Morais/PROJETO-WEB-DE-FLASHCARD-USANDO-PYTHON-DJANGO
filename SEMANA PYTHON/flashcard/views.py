...
def novo_flashcard(request):
    ...
    if request.method == 'GET':
        ...
        if len(pergunta.strip()) == 0 or len(resposta.strip()) == 0: ##STRIP TIRA O ESPAÇO EM BRANCO DA STRING EM PYTHON 
            messages.add_message(                                    ##NAO DIXAR O USUARIO CADASTRAR ESPAÇOS EM BRANCO##
                request,
                constants.ERROR,
...
def iniciar_desafio(request):
    ...
    elif request.method == 'POST': #pegando os dados do html#
...
def desafio(request, id):
    ...
    if not desafio.user == request.user:##verificar se é o usuario certo
        raise Http404()
    ...
    if request.method == 'GET':
        ...
        return render(request, 'desafio.html',{'desafio': desafio, 'acertos': acertos, 'faltantes': faltantes,'erros' : erros})
...
def responder_flashcard(request, id):
    ...
    if not flashcard_desafio.flashcard.user == request.user:##verificar se é o usuario certo
        raise Http404()
